...
void deleteOwned(PropLList *L, TitleDeed T);
...
void readPlayerOwnedProp(int player_id);
...
int main(int argc, char *argv[]) {
	...
	storeProperty(G, 20103264);
	storeProperty(G, 1);
	storeProperty(G, 2);
	readPlayerOwnedProp(20103264);
	readPlayerOwnedProp(1);
	readPlayerOwnedProp(2);
	...
	if(x != board->playerCount) {
		...
		if(scan != NULL) {
			if(board->players[x].totalMoney >= scan->lot.amount[0]) {
				...
				deleteOwned(&(board->ownedBank), scan->lot);
			...
			if(found) {
				if(scan3->lot.houseCount == 0 && scan3->lot.hotelCount == 0 && board->players[x].totalMoney >= scan3->lot.amount[0] * 3) {
					...
					deleteOwned(&(board->players[y]).owned, scan3->lot);
					...
					insertSorted(&(board->players[x].owned), scan3->lot);
					deleteOwned(&(board->players[y-1]).owned, scan3->lot);
...
bool storeProperty(GameBoard board, int player_id) {
	...
	if(x != board.playerCount) {
		...
		strcat(filename, itoa(player_id, buffer, 10));
		strcat(filename,"_titledeeds.cis");
		storeFp = fopen(filename, "wb");
		for(current = board.players[x].owned; current != NULL; current = current->next) {
			fwrite(&(current->lot), sizeof(TitleDeed), 1, storeFp);
		}
		fclose(storeFp);
	}
}
...
void readPlayerOwnedProp(int player_id) {
	...
	if(readFp != NULL) {
		for(x = 0; x < 22 && fread(&titles.lot[x], sizeof(TitleDeed), 1, readFp); x++) {
			printf("%2d - %30s - %15s\n", titles.lot[x].id, titles.lot[x].name, titles.lot[x].color);
		}
		printf("\n");
	}
	fclose(readFp);

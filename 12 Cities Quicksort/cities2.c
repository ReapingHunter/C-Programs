...
void quicksort(struct City Cities[], int length);
void quicksortRecursion(struct City Cities[], int low, int high);
int partition(struct City Cities[], int low, int high);
...
int main (void){
	...
	quicksortRecursion(Cities, 0, row - 1);
	//27.1 seconds
...
void quicksortRecursion(struct City Cities[], int low, int high) {
	int pivot;
	if(low < high) {
		pivot = partition(Cities, low, high);
		quicksortRecursion(Cities, low, pivot - 1);
		quicksortRecursion(Cities, pivot + 1, high);
	}
}

int partition(struct City Cities[], int low, int high){
	float pivotValue = Cities[high].latitude;
	int i = low;
	int j;
	for(j = low; j < high; j++) {
		if(Cities[j].latitude <= pivotValue) {
			swap(&Cities[i], &Cities[j]);
			i++;
		}
	}
	swap(&Cities[i], &Cities[high]);
	return i;
}

	...
	for(i = 0; i < MAX_RACK; i++){
		...
		if(vm.shelves[i] == FALSE){
			...
			for(j = 0; j < vm.rackItems[i].itemCount; j++){
				printf("\n\tItem %d - \t%10s - Php %10.2f - %02d/%02d/%02d", j, vm.rackItems[i].list[j].prodName, vm.rackItems[i].list[j].prodPrice, vm.rackItems[i].list[j].expiryDate.day, vm.rackItems[i].list[j].expiryDate.month, vm.rackItems[i].list[j].expiryDate.year);
			}
		    printf("\n");
		}
...
Boolean addProduct(VendingMachine *vm, int rackNumber, Product prodItem) {
	...
	if(rackNumber < MAX_RACK && vm->rackItems[rackNumber].itemCount < MAX_ITEM){
		printf("Successfully Added.\n");
		vm->shelves[rackNumber] = TRUE;
		vm->rackItems[rackNumber].list[vm->rackItems[rackNumber].itemCount++] = prodItem;
		for(i = 0; i < MAX_RACK; i++){
			for(j = 0; j < vm->rackItems[i].itemCount-1; j++){
				if(vm->rackItems[i].list[j].expiryDate.day > vm->rackItems[i].list[j+1].expiryDate.day){
					temp = vm->rackItems[i].list[j];
					vm->rackItems[i].list[j] = vm->rackItems[i].list[j+1];
					vm->rackItems[i].list[j+1] = temp;
				}
			}
		}
		return TRUE;
		...
		printf("ERROR: Rack non existent or items full.\n");
		return FALSE;
	}
...
Boolean purchaseProduct(VendingMachine *vm, int rackNumber, float amount) {
	...
	if(vm->rackItems[rackNumber].itemCount == 0){
		...
		printf("You have a change of %.2f\n",amount - vm->rackItems[rackNumber].list[0].prodPrice);
		vm->totalMoney += vm->rackItems[rackNumber].list[0].prodPrice;
		memmove(&vm->rackItems[rackNumber].list[0],&vm->rackItems[rackNumber].list[1],sizeof(Product)*(vm->rackItems[rackNumber].itemCount--));
		if(vm->rackItems[rackNumber].itemCount == 0)

...
void insertSorted(cursorSet *A, product P) {
    int x;
    int avail;
    avail = mallocInVHeap(A->VHptr);
    if(avail != -1) {
    	A->VHptr->VH_node[avail].elem = P;
    	if(A->elemIndex == -1) {
    		A->VHptr->VH_node[avail].next = A->elemIndex;
    		A->elemIndex = avail;
		} else {
			for(x = A->elemIndex; x != -1 && strcmp(A->VHptr->VH_node[x].elem.prodID, P.prodID) < 0; x = A->VHptr->VH_node[x].next){}
			A->VHptr->VH_node[avail].next = A->VHptr->VH_node[x].next;
			A->VHptr->VH_node[x].next = avail;
		}
		A->count += 1;
	}
